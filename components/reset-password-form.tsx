...
function translateAuthError(errorMessage: string): string {
  const errorTranslations: Record<string, string> = {
    ...
    "Auth session missing":
      "Sesión no encontrada. Por favor, solicite un nuevo enlace de restablecimiento.",
...
export function ResetPasswordForm({
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Also activate the user account if not already activated
      // This handles the case where a user who clicked the invite link but didn't
      // complete setup is sent a password reset email as a fallback
      console.log("[ResetPasswordForm] Calling activateUserAccount...")
      const activationResult = await activateUserAccount()
      console.log("[ResetPasswordForm] activateUserAccount result:", activationResult)
      if (activationResult.error) {
        console.error("[ResetPasswordForm] Error activating account:", activationResult.error)
        // Don't block the user - password was set successfully
      }

      toast.success("Contraseña actualizada exitosamente")

      // Redirect to home after password reset - use hard navigation for cache clearing
