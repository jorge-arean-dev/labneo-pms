...
import type { MedicoDashboardData, DashboardConsulta, DashboardPaciente } from "../actions"

interface MedicoDashboardProps {
  data: MedicoDashboardData
}
...
/**
 * Consulta row component for today's appointments
 * The tick parameter forces re-render to update delay time
 */
function ConsultaHoyRow({ consulta, tick }: { consulta: DashboardConsulta; tick: number }) {
...
/**
 * Consulta row component for next 7 days (includes date)
 */
function ConsultaProximaRow({ consulta }: { consulta: DashboardConsulta }) {
...
export function MedicoDashboard({ data }: MedicoDashboardProps) {
  const router = useRouter()

  // Local state for dashboard data - allows real-time updates without full page refresh
  const [dashboardData, setDashboardData] = useState<MedicoDashboardData>(data)

  // Tick state for auto-updating delay times every 60 seconds
  const [tick, setTick] = useState(0)

  // Destructure from local state for reactivity
  const { medicoId, medicoNombre, pacientesEnEspera, consultasHoy, consultasProximos7Dias, pacientesRecientes } = dashboardData
  ...
  // Note: We listen to ALL consultas updates and filter client-side by medicoId
  // because Supabase Realtime filters only work reliably on primary key columns
  useEffect(() => {
    const supabase = createClient()
    let channel: ReturnType<typeof supabase.channel> | null = null

    console.log("[Dashboard Realtime] Setting up subscription for medicoId:", medicoId)
    ...
    const setupSubscription = async () => {
      ...
      console.log("[Dashboard Realtime] Session exists:", !!session)
      if (session?.access_token) {
        await supabase.realtime.setAuth(session.access_token)
        console.log("[Dashboard Realtime] Set realtime auth token")
      }

      // Use unique channel name to avoid conflicts with other realtime subscriptions
      const channelName = `dashboard-consultas-${medicoId}-${Date.now()}`
      channel = supabase
        .channel(channelName)
        .on(
          "postgres_changes",
          ...
          (payload) => {
            ...
            console.log("[Dashboard Realtime] Received event:", payload.eventType, "id:", newRecord?.id || oldRecord?.id)

            // Only process UPDATE events for now
            if (payload.eventType !== "UPDATE") {
              console.log("[Dashboard Realtime] Non-UPDATE event, triggering refresh")
              // For INSERT/DELETE, check médico and refresh
              const relevantMedicoId = newRecord?.medico_id || oldRecord?.medico_id
              if (relevantMedicoId === medicoId) {
                router.refresh()
              }
              return
            }

            console.log("[Dashboard Realtime] Received UPDATE event:", {
              consultaId: newRecord.id,
              payloadMedicoId: newRecord.medico_id,
              currentMedicoId: medicoId,
              matches: newRecord.medico_id === medicoId,
            })

            // Filter client-side: only process updates for this médico's consultas
            if (newRecord.medico_id !== medicoId) {
              console.log("[Dashboard Realtime] Skipping - different médico")
              return
            }

            console.log("[Dashboard Realtime] Processing consulta update:", newRecord.id)

            // Update consultasHoy if the consulta is in the list
        ...
        .subscribe((status, err) => {
          ...
          if (status === "SUBSCRIBED") {
            console.log("[Dashboard Realtime] Successfully subscribed to postgres_changes on consultas table")
          }
          if (status === "CHANNEL_ERROR") {
            console.error("[Dashboard Realtime] Channel error occurred")
          }
        })
    }
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Hola {medicoNombre} 👋</h1>
        ...
        <Card>
          ...
          <CardContent>
            {consultasHoy.length > 0 ? (
              ...
              <EmptyState
                message="No tienes consultas programadas para hoy"
                icon={CalendarDays}
              />
            )}
        ...
        <Card>
          ...
          <CardContent>
            {pacientesRecientes.length > 0 ? (
              ...
              <EmptyState
                message="No han habido consultas en los últimos 7 días"
                icon={Users}
              />
            )}
      ...
      <Card>
        ...
        <CardContent>
          {consultasProximos7Dias.length > 0 ? (
            ...
            <EmptyState
              message="No tienes consultas programadas para los próximos 7 días"
              icon={Clock}
            />
          )}
